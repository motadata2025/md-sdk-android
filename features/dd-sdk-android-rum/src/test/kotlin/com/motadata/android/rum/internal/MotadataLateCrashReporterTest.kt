...
import com.motadata.android.rum.assertj.ErrorEventAssert
import com.motadata.android.rum.assertj.ViewEventAssert
...
import com.motadata.android.rum.internal.anr.AndroidTraceParser
...
internal class MotadataLateCrashReporterTest {
    ...
    fun `set up`() {
        ...
        testedHandler = MotadataLateCrashReporter(
            sdkCore = mockSdkCore,
    ...
    fun `M send RUM view+error W handleNdkCrashEvent()`(
        ...
        val fakeViewEvent = viewEvent.copy(
            date = fakeCurrentTimeMs - forge.aLong(
                min = 0L,
                max = MotadataLateCrashReporter.VIEW_EVENT_AVAILABILITY_TIME_THRESHOLD - 1000
            ),
            usr = ViewEvent.Usr(
                id = fakeUserInfo.id,
    ...
    fun `M send only RUM error W handleNdkCrashEvent() {view is too old}`(
        ...
        val fakeViewEvent = viewEvent.copy(
            date = fakeCurrentTimeMs - forge.aLong(
                min = MotadataLateCrashReporter.VIEW_EVENT_AVAILABILITY_TIME_THRESHOLD + 1
            ),
            usr = ViewEvent.Usr(
                id = fakeUserInfo.id,
                name = fakeUserInfo.name,
    ...
    fun `M not send RUM event W handleNdkCrashEvent() { RUM feature is not registered }`(
        ...
        mockInternalLogger.verifyLog(
            ...
            MotadataLateCrashReporter.INFO_RUM_FEATURE_NOT_REGISTERED
        )
    }
    ...
    fun `M not send RUM event W handleNdkCrashEvent() { corrupted event, view json deserialization fails }`(
        ...
        mockInternalLogger.verifyLog(
            ...
            InternalLogger.Target.USER,
            MotadataLateCrashReporter.NDK_CRASH_EVENT_MISSING_MANDATORY_FIELDS
        )
    }
    ...
    fun `M send only RUM error W handleAnrCrash() { view is too old }`(
        ...
        val fakeViewEvent = viewEvent.copy(
            date = fakeCurrentTimeMs - forge.aLong(
                min = MotadataLateCrashReporter.VIEW_EVENT_AVAILABILITY_TIME_THRESHOLD + 1,
                max = MotadataLateCrashReporter.VIEW_EVENT_AVAILABILITY_TIME_THRESHOLD * 2
            ),
            usr = ViewEvent.Usr(
                id = fakeUserInfo.id,
                name = fakeUserInfo.name,
    ...
    fun `M not send anything W handleAnrCrash() { cannot open trace information, IOException }`(
        ...
        argumentCaptor<() -> String> {
            ...
            assertThat(firstValue()).isEqualTo(MotadataLateCrashReporter.OPEN_ANR_TRACE_ERROR)
        }
    }
    ...
    fun `M not send anything W handleAnrCrash() { cannot open trace information, null }`(
        ...
        argumentCaptor<() -> String> {
            ...
            assertThat(firstValue()).isEqualTo(MotadataLateCrashReporter.MISSING_ANR_TRACE)
        }
    }
