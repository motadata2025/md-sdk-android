...
internal class MotadataLogGeneratorTest {

    lateinit var testedLogGenerator: MotadataLogGenerator
    ...
    fun `set up`(forge: Forge) {
        ...
        testedLogGenerator = MotadataLogGenerator(
            fakeServiceName,
            mockInternalLogger
        )
    }
    ...
    fun `M add the service name from Motadata context W creating the Log { no service name }`() {
        // WHEN
        testedLogGenerator = MotadataLogGenerator(internalLogger = mockInternalLogger)
        val log = testedLogGenerator.generateLog(
            fakeLevel,
            fakeLogMessage,
            fakeThrowable,
    ...
    fun `M log warning and drop W user tag matches reserved`(
        ...
        // Given
        val fakeUserReservedTags = MotadataLogGenerator.RESERVED_TAG_KEYS
        ...
        argumentCaptor<() -> String> {
            verify(mockInternalLogger, times(MotadataLogGenerator.RESERVED_TAG_KEYS.size))
                .log(
            ...
            val expectedMessages = fakeUserReservedTags.map {
                MotadataLogGenerator.RESERVED_TAG_DROPPED_WARNING.format(
                    Locale.US,
                    it
                )
            }
    ...
    fun `M use status DEBUG W creating the Log { other level }`(forge: Forge) {
        // WHEN
        val log = testedLogGenerator.generateLog(
            forge.anInt(min = MotadataLogGenerator.CRASH + 1),
            fakeLogMessage,
            fakeThrowable,
            fakeAttributes,
            fakeTags,
