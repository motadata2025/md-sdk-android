...
import com.motadata.android.api.instrumentation.network.ExtendedRequestInfo
...
internal class MotadataPropagationHelperTest {

    private lateinit var testedHelper: MotadataPropagationHelper
    ...
    @Mock
    lateinit var mockTraceId: MotadataTraceId
    ...
    @BeforeEach
    fun `set up`() {
        testedHelper = MotadataPropagationHelper()
    ...
    fun `M return true W isExtractedContext() {extracted context}`(forge: Forge) {
        ...
        val context = MotadataSpanContextAdapter(extractedContext)
    ...
    @Test
    fun `M return false W isExtractedContext() {non-extracted MotadataSpanContextAdapter}`() {
        // Given
        val mockAgentSpanContext = mock<AgentSpan.Context>()
        val context = MotadataSpanContextAdapter(mockAgentSpanContext)
    ...
    @Test
    fun `M return false W isExtractedContext() {non-MotadataSpanContextAdapter}`(
        @Forgery fakeContext: MotadataSpanContext
    ) {
        // When
    ...
    fun `M create extracted context W createExtractedContext()`(
        ...
        // Then
        assertThat(result).isInstanceOf(MotadataSpanContextAdapter::class.java)
    ...
    @Test
    fun `M return span context from tag W extractParentContext() {MotadataSpan tag}`() {
        // Given
    ...
    fun `M return extracted header context W extractParentContext() {extracted context from headers}`(
        ...
        val extractedSpanContext = MotadataSpanContextAdapter(extractedContext)
    ...
    fun `M return tag context W extractParentContext() {non-extracted header context}`() {
        // Given
        val mockRequest = createMockRequestWithTags(datadogSpan = mockSpan)
        val nonExtractedContext: MotadataSpanContext = mock()
    ...
    @Test
    fun `M return sampled from span W extractSamplingDecision() {MotadataSpan tag with positive priority}`() {
        // Given
    ...
    @Test
    fun `M return not sampled from span W extractSamplingDecision() {MotadataSpan tag with zero priority}`() {
        // Given
    ...
    fun `M remove Motadata headers W propagateNotSampledHeaders() {SAMPLED injection type}`() {
        ...
        // Then
        MotadataPropagationHelper.DATADOG_CODEC_HEADERS.forEach { headerKey ->
            verify(mockRequestModifier).removeHeader(headerKey)
        }
    }
    ...
    fun `M remove B3Multi headers W propagateNotSampledHeaders() {SAMPLED injection type}`() {
        ...
        // Then
        MotadataPropagationHelper.B3M_CODEC_HEADERS.forEach { headerKey ->
            verify(mockRequestModifier).removeHeader(headerKey)
        }
    }
    ...
    fun `M remove W3C headers W propagateNotSampledHeaders() {SAMPLED injection type}`() {
        ...
        // Then
        MotadataPropagationHelper.W3C_CODEC_HEADERS.forEach { headerKey ->
            verify(mockRequestModifier).removeHeader(headerKey)
        }
    }
    ...
    fun `M remove all tracing headers W removeAllTracingHeaders()`() {
        ...
        // Then
        MotadataPropagationHelper.ALL_TRACING_HEADERS.forEach { headerKey ->
            verify(mockRequestModifier).removeHeader(headerKey)
        }
    }
    ...
    fun `M add drop sampling B3 header W propagateNotSampledHeaders() {ALL injection type, B3}`() {
        ...
        // Then
        verify(mockRequestModifier).addHeader(B3HttpCodec.B3_KEY, MotadataPropagationHelper.B3_DROP_SAMPLING_DECISION)
    }
    ...
    fun `M add drop sampling B3Multi header W propagateNotSampledHeaders() {ALL injection type, B3MULTI}`() {
        ...
        verify(mockRequestModifier).addHeader(
            ...
            MotadataPropagationHelper.B3M_DROP_SAMPLING_DECISION
        )
    }

    private fun createMockRequestWithTags(
        headers: Map<String, List<String>> = emptyMap(),
        datadogSpan: MotadataSpan? = null,
        traceContext: TraceContext? = null
    ): HttpRequestInfo {
        return mock<HttpRequestInfoWithTags> {
            on { this.headers } doReturn headers
            on { tag(MotadataSpan::class.java) } doReturn datadogSpan
