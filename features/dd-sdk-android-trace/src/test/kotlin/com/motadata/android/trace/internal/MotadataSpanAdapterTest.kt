...
internal class MotadataSpanAdapterTest {
    ...
    private lateinit var testedSpanAdapter: MotadataSpanAdapter
    private lateinit var testedSpanAdapterWithDDSpanDelegate: MotadataSpanAdapter

    @BeforeEach
    fun `set up`() {
        whenever(mockAgentSpan.context()).thenReturn(mockSpanContext)
        whenever(mockAgentSpan.traceId).thenReturn(mockDDTraceId)
        testedSpanAdapter = MotadataSpanAdapter(mockAgentSpan, mockSpanLogger)
        testedSpanAdapterWithDDSpanDelegate = MotadataSpanAdapter(mockDDSpan, mockSpanLogger)
    }
    ...
    fun `M return MotadataContext instance W context() is called`() {
        ...
        // Then
        assertThat(context).isInstanceOf(MotadataSpanContext::class.java)
    }

    @Test
    fun `M return MotadataTraceId instance W traceId is called`() {
        // When
        val traceId = testedSpanAdapter.traceId

        // Then
        assertThat(traceId).isInstanceOf(MotadataTraceId::class.java)
    }
