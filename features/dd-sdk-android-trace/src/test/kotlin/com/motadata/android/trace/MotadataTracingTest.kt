...
import com.motadata.android.trace.MotadataTracing.ErrorMessages.DEFAULT_SERVICE_NAME_IS_MISSING_ERROR_MESSAGE
import com.motadata.android.trace.MotadataTracing.ErrorMessages.TRACING_NOT_ENABLED_ERROR_MESSAGE
import com.motadata.android.trace.MotadataTracing.ErrorMessages.WRITER_PROVIDER_INTERFACE_NOT_IMPLEMENTED_ERROR_MESSAGE
import com.motadata.android.trace.MotadataTracing.ErrorMessages.buildWrongWrapperMessage
...
class MotadataTracingTest {
    ...
    fun `set up`(forge: Forge) {
        ...
        val writerWrapperMock = mock<MotadataSpanWriterWrapper> {
    ...
    fun `M use a NoOpCoreTracerWriter W build { TracingFeature not enabled }`() {
        ...
        // When
        val tracer = MotadataTracing.newTracerBuilder(mockSdkCore).build() as MotadataTracerAdapter
    ...
    fun `M log a maintainer error W build { TracingFeature not implementing InternalCoreWriterProvider }`() {
        ...
        // When
        val tracer = MotadataTracing.newTracerBuilder(mockSdkCore).build()
    ...
    fun `M log a user error W build { default service name not available }`() {
        ...
        // When
        MotadataTracing.newTracerBuilder(mockSdkCore).build()
    ...
    fun `M log a user error W build { writer is null }`() {
        // Given
        class CustomWrapper : MotadataSpanWriter
