...
internal class MotadataPropagationAdapterTest {

    private lateinit var testedPropagation: MotadataPropagationAdapter
    ...
    @BeforeEach
    fun `set up`() {
        testedPropagation = MotadataPropagationAdapter(
            mockInternalLogger,
    ...
    @Test
    fun `M report an error W inject { unsupported MotadataSpanContext implementation provided }`() {
        // Given
        val carrier = Any()
    ...
    fun `M delegate to AgentPropagation W inject `() {
        // Given
        val carrier = Any()
        val mockContext: MotadataSpanContextAdapter = mock {
    ...
    @Test
    fun `M return MotadataSpanContextAdapter W extract `() {
        // Given
        val carrier = Any()
        ...
        // When
        val actual = testedPropagation.extract(carrier, getter) as MotadataSpanContextAdapter
