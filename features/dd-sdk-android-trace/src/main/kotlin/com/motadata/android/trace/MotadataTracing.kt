...
object MotadataTracing {
    /**
     * Creates and returns a new instance of [MotadataTracerBuilder]. This method initializes and configures the
     ...
     * the tracer builder.
     * @return A configured instance of [MotadataTracerBuilder], or a no-operation implementation if the necessary
     * configurations or features are unavailable.
     */
    @JvmStatic
    fun newTracerBuilder(sdkCore: SdkCore = Motadata.getInstance()): MotadataTracerBuilder = when {
        ...
        sdkCore !is FeatureSdkCore -> {
            ...
            val writer = (internalCoreWriterProvider?.getCoreTracerWriter() as? MotadataSpanWriterWrapper)?.delegate
    ...
    internal object ErrorMessages {
        const val TRACING_NOT_ENABLED_ERROR_MESSAGE: String =
            "You're trying to create an MotadataTracer instance, " +
        ...
        const val DEFAULT_SERVICE_NAME_IS_MISSING_ERROR_MESSAGE: String =
            "Default service name is missing during MotadataTracerBuilder creation, did you initialize SDK?"

        fun buildWrongWrapperMessage(cls: Class<*>?): String {
            return "You're trying to create an MotadataTracer instance, " +
