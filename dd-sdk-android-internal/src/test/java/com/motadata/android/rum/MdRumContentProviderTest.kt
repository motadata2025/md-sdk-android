...
import com.motadata.android.internal.forge.Configurator
...
class MdRumContentProviderTest {
    ...
    fun `set up`(forge: Forge) {
        ...
        testedProvider = MdRumContentProvider()
        testedProvider.setFieldValue("mContext", mockContext)
    }

    @AfterEach
    fun `tear down`() {
        MdRumContentProvider.processImportance = 0
    }
    ...
    fun `M detect process importance W onCreate()`(
        ...
        // Then
        assertThat(MdRumContentProvider.processImportance).isEqualTo(processImportance)
    }
    ...
    fun `M detect process importance once W onCreate() twice`(
        ...
        // Then
        assertThat(MdRumContentProvider.processImportance).isEqualTo(processImportance1)
    }
    ...
    fun `M detect default process importance W onCreate() {no context}`() {
        ...
        // Then
        assertThat(MdRumContentProvider.processImportance)
            .isEqualTo(MdRumContentProvider.DEFAULT_IMPORTANCE)
    }
