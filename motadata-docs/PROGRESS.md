...
**FINALIZE — DONE:** ✅ api-surface regen (bot `7b5c1a70e`, run 27130296119 — captures `QUERY_PARAM_API_KEY` const + public `allowClearTextHttp()` + `created`×7 + `isViewCompleted` in generated models) · ✅ version bump → `1.0.1` (`ba15aa1b3`) · ✅ published 7 modules to GitHub Packages as `1.0.1` (run 27131011003; `com.motadata:motadata-rum-android*:1.0.1` live alongside `1.0.0`). Publish workflow made branch-agnostic (`ae21bc49e`).

**✅ ON-DEVICE VERIFIED (2026-06-08) — BRANCH 2 SDK-SIDE SIGNED OFF.** 20 batches / 196 events (`temp/rum-captures/android/20260608-*`, 4 sessions): `md-api-key` in **all 20/20** request queries (header `MD-EVP-ORIGIN-VERSION:1.0.1`); `view.is_view_completed` flips `"yes"`×8/`"no"`×23; `session.created` + `context._timing` on all **187** backend-consumed events, **absent** on the 4 `vital/app_launch` + 5 `telemetry` (by design); `navigationStart == created` always (Option A); `relativeTime` in **nanoseconds** (correct magnitude); **0** `_dd`/`datadog` leaks; all 7 event types present. **Edge case (DECIDED: KEEP as-is, option B):** the `ApplicationLaunch` view (once per session) shows **negative** `relativeTime` (~-1 to -2.5s) because its `date` is backdated to app-start (before `session.created`); only that one synthetic view, every other event positive. **Remaining = user's manual checks only:** Logcat `Motadata SDK initialized`; end-to-end auth (200 not 401 + ingested with a valid registered token). Field units now in `EVENT_TYPES.md` §3.1.
