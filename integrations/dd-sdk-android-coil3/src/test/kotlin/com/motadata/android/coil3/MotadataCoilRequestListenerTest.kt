...
internal class MotadataCoilRequestListenerTest {

    private lateinit var testable: MotadataCoilRequestListener
    ...
    fun `set up`() {
        ...
        testable = MotadataCoilRequestListener(mockSdkCore)
    }
    ...
    fun `M send RUM error event W raw string path Request fails`(forge: Forge) {
        ...
        verify(mockRumMonitor).addError(
            eq(MotadataCoilRequestListener.REQUEST_ERROR_MESSAGE),
            eq(RumErrorSource.SOURCE),
            eq(fakeException),
            argumentCaptor.capture()
        )
        assertThat(argumentCaptor.firstValue).containsEntry(
            MotadataCoilRequestListener.REQUEST_PATH_TAG,
            fakePath
        )
    }
