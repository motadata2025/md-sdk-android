...
class MotadataSqliteCallbackTest {

    lateinit var testedSqliteCallback: MotadataSqliteCallback
    ...
    fun `set up`() {
        ...
        testedSqliteCallback = MotadataSqliteCallback(mock(), mockSdkCore)
    ...
    fun `M send an error event W intercepting a DB corruption`() {
        ...
        verify(mockRumMonitor).addError(
            eq(
                String.format(
                    Locale.US,
                    MotadataSqliteCallback.DATABASE_CORRUPTION_ERROR_MESSAGE,
                    fakeDbPath
                )
            ),
