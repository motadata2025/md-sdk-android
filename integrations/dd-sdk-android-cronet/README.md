# Motadata SDK instrumentation for Cronet

This module provides Motadata instrumentation for [Cronet](https://developer.android.com/guide/topics/connectivity/cronet) HTTP requests. It supports:
...
**Note**: Motadata uses `cronet-api` version `141.7340.3` and has verified compatibility with the `play-services-cronet` implementation at version `17.0.1`.
...
**OkHttp equivalent:** `addInterceptor(MotadataInterceptor)`
...
**OkHttp equivalent:** `addInterceptor(MotadataInterceptor)` + `addNetworkInterceptor(TracingInterceptor)`
...
When `apmInstrumentationConfiguration` is provided, RUM-APM linking is enabled by default — the SDK injects `x-datadog-trace-id` and `x-datadog-parent-id` headers into outgoing requests, allowing you to navigate between RUM Resources and APM Spans in the Motadata UI and build end-to-end distributed traces across client, backend, and other components.
...
**OkHttp equivalent:** `addInterceptor(MotadataInterceptor)` without `Trace.enable(tracesConfig)`
