...
import com.motadata.android.cronet.CronetIntegrationPlugin.Companion.CRONET_NETWORK_INSTRUMENTATION_NAME
...
import com.motadata.android.rum.internal.net.RumNetworkInstrumentationAssert
...
internal class MotadataCronetEngineBuilderTest {
    @Mock
    ...
    @Test
    fun `M create MotadataCronetEngine with APM W build() { only apmInstrumentationConfiguration }`() {
        // When
        val engine = mockBuilderDelegate
            .configureDatadogInstrumentation(
    ...
    @Test
    fun `M create MotadataCronetEngine with both W build() { apm + rum configurations }`() {
        // When
        val engine = mockBuilderDelegate
            .configureDatadogInstrumentation(
                rumInstrumentationConfiguration = RumNetworkInstrumentationConfiguration(),
    ...
    fun `M return plain CronetEngine W build() { no instrumentation }`() {
        ...
        // Then
        assertThat(engine).isNotInstanceOf(MotadataCronetEngine::class.java)
