...
internal class MotadataFrescoCacheListenerTest {

    private lateinit var underTest: MotadataFrescoCacheListener
    ...
    fun `set up`() {
        ...
        underTest = MotadataFrescoCacheListener(mockSdkCore)
    }
    ...
    fun `M add RUM Error event W onReadException()`(forge: Forge) {
        ...
        verify(mockRumMonitor).addError(
            eq(MotadataFrescoCacheListener.CACHE_ERROR_READ_MESSAGE),
            eq(RumErrorSource.SOURCE),
            eq(fakeException),
            argumentCaptor.capture()
        )
        assertThat(argumentCaptor.firstValue).containsEntry(
            MotadataFrescoCacheListener.CACHE_ENTRY_URI_TAG,
            fakeCacheEventUri
        )
    }
    ...
    fun `M add RUM Error W onWriteException()`(forge: Forge) {
        ...
        val argumentCaptor = argumentCaptor<Map<String, Any?>>()
        verify(mockRumMonitor).addError(
            eq(MotadataFrescoCacheListener.CACHE_ERROR_WRITE_MESSAGE),
            eq(RumErrorSource.SOURCE),
            eq(fakeException),
            argumentCaptor.capture()
        )
