...
package com.motadata.android.apollo
...
import com.motadata.android.apollo.internal.VariablesExtractor
import com.motadata.android.internal.network.GraphQLHeaders
import com.motadata.android.internal.utils.toBase64
...
@Suppress("UnusedFlow")
internal class MotadataApolloInterceptorTest {
    ...
    private lateinit var testedInterceptor: MotadataApolloInterceptor
    ...
    fun `M handle null variables W intercept { variables extractor returns null }`() {
        ...
        val interceptor = MotadataApolloInterceptor(
    ...
    fun `M use default variables extractor W intercept { no custom extractor provided }`() {
        // Given
        val defaultInterceptor = MotadataApolloInterceptor()
    ...
    fun `M send payload W intercept { sendGraphQLPayloads true }`() {
        // Given
        val interceptor = MotadataApolloInterceptor(sendGraphQLPayloads = true)
        val (_, originalRequest, requestBuilder) = setupBasicMocks("GetUser", "query GetUser { user { id: name } }")
        val chain = mock<ApolloInterceptorChain>()
    ...
    fun `M not send payload W intercept { sendGraphQLPayloads false }`() {
        // Given
        val interceptor = MotadataApolloInterceptor(sendGraphQLPayloads = false)
        val (_, originalRequest, requestBuilder) = setupBasicMocks("GetUser", "query GetUser { user { id: name } }")
        val chain = mock<ApolloInterceptorChain>()
    ...
    private fun setupApolloInterceptor(sendGraphQLPayloads: Boolean): MotadataApolloInterceptor {
        ...
        return MotadataApolloInterceptor(
