...
import com.motadata.android.core.internal.NoOpContextProvider
...
/**
 * This region groups all test about MotadataCore instance (except Initialization).
 */
@Extensions(
...
internal class MotadataCoreTest {

    private lateinit var testedCore: MotadataCore
    ...
    fun `M notify no feature registered W setEventReceiver() { feature is not registered }`(
        ...
        mockInternalLogger.verifyLog(
            ...
            MotadataCore.MISSING_FEATURE_FOR_EVENT_RECEIVER.format(Locale.US, feature)
        )
    }
    ...
    fun `M notify receiver exists W setEventReceiver() { feature already has receiver }`(
        ...
        mockInternalLogger.verifyLog(
            InternalLogger.Level.WARN,
            InternalLogger.Target.USER,
            MotadataCore.EVENT_RECEIVER_ALREADY_EXISTS.format(Locale.US, feature)
        )
    }
    ...
    fun `M notify listener already registered W setContextUpdateReceiver()`() {
        ...
        mockInternalLogger.verifyLog(
            InternalLogger.Level.WARN,
            InternalLogger.Target.USER,
            MotadataCore.CONTEXT_UPDATE_LISTENER_ALREADY_REGISTERED.format(Locale.US, mockContextUpdateListener)
        )
    }
    ...
    fun `M log info when writing last view event W writeLastViewEvent(){ below R and no NDK feature }`(
        ...
        mockInternalLogger.verifyLog(
            ...
            MotadataCore.NO_NEED_TO_WRITE_LAST_VIEW_EVENT
        )
    }
